...
export const TransactionList = () => {
    ...
    const {isReady, transactions} = useTracker(() => {
        ...
        const transactionsResult = TransactionsCollection.find({}, {sort: {date: -1}}).fetch();
        ...
        const groupedByDate = groupBy(transactionsResult, (transaction) => isPastOrToday(transaction.date) ? "past" : "future");
        const transactions = {
            future: groupedByDate.future,
            past: groupedByDate.past,
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {transactions.past && (
                                        <Fragment>
                                            <TableRow>
                                                <TableCell className="text-bold" colSpan={4}>Realizadas</TableCell>
                                            </TableRow>
                                            {transactions.past.map(transactionRow)}
                                        </Fragment>
                                    )}
                                    {transactions.future && (
                                        <Fragment>
                                            <TableRow>
                                                <TableCell className="text-bold" colSpan={4}>Futuras</TableCell>
                                            </TableRow>
                                            {transactions.future.map(transactionRow)}
                                        </Fragment>
                                    )}
                                </TableBody>
                                <TableFooter>
